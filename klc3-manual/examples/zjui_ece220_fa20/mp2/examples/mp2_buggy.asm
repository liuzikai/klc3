...
INIT_SCHED
  ...
  INIT_SCHED_LOOP
    BRn INIT_SCHED_DONE  ; NOTE: should be BRz, off by one, access unspecified memory
...
FILL_DAYS
  ...
  ADD R6,R6,#1 ; R6 = 1
  AND R7,R7,#0
  ADD R7,R7,#4 ; R7 holds the remaining weekdays to process ; NOTE: should be 5, off by one
  FD_CHECK_LOOP
    BRz FD_CHECK_DONE
