...
;      of schedule at x4000. If any invalid slot numbers or conflict is
...
; INIT_SCHED
;   Initialize the fixed-length field of schedule at x4000.
;   Input: None
;   Output: None
...
TRANS_SCHED
  ...
  ST R7,TS_R7 ; store R7 for RET

  LD R1,LIST_ADDR
  TS_EVENT_LOOP

    ADD R2,R1,#0  ; store the starting pointer of the event to R2
    ...
    ADD R7,R7,#0  ; set the condition codes
    BRz TS_EVENT_DONE

    LDR R3,R1,#1  ; load the bit vector of the event
    LDR R4,R1,#2  ; load the slot index of the event
    ADD R1,R1,#3  ; point R1 to the start of the label of next event

    ; check the slot number
    ADD R4,R4,#0  ; set the condition codes
    BRn TS_EVENT_CHECK_SLOT_FAIL  ; if R4 < 0, fails
    ADD R0,R4,#-15
    BRp TS_EVENT_CHECK_SLOT_FAIL  ; if R4 > 15, fails
...
SCHED_ADDR .FILL x4000
SCHED_LENGTH .FILL #80

LIST_ADDR .FILL x5000

EXTRA_LIST_ADDR .FILL x6000
...
FILL_DAYS
  ...
  ; make R5 point to the Mon of the slot of current event
  LD R5,SCHED_ADDR
  ADD R4,R4,#0  ; set the condition codes
  FD_INIT_R5_LOOP
    BRz FD_INIT_R5_DONE
  ...
  FD_CHECK_LOOP
    ...
    FD_CHECK_NEXT
    ADD R5,R5,#1  ; make R5 points to the next day
    ADD R6,R6,R6  ; R6 <<= 1
    ADD R7,R7,#-1
    BRnzp FD_CHECK_LOOP
  FD_CHECK_DONE

  ADD R5,R5,#-5  ; recover R5 to the start of current slot
...
;     R6 + 2: pointer to event str pointer
DFS
  ...
  LD R7,EXTRA_LIST_ADDR  ; load the first extra event

  DFS_PUSH  ; push the event pointed by R7 to the stack

    LDR R0,R7,#0
    BRz DFS_SUCCESS  ; if R7 == NULL, no more event needs to be handled
  ...
  DFS_TRIAL
    ...
    LDR R3,R7,#1  ; load the day vector of current event
    ...
    LDR R7,R6,#2  ; load the extra event pointer
    LDR R2,R7,#0  ; load the str pointer of current event
    LDR R1,R7,#2  ; load the slot vector of current event

    DFS_TRY_SLOT_LOOP

      ADD R0,R4,#-15
      BRp DFS_TRY_SLOT_DONE  ; if R4 > 15, this event is not compatible
...
PRINT_SCHED
  ...
  PS_HEADER_DAY_LOOP
    ...
    ADD R1,R1,#4  ; each string use 4 memory locations (including NUL)
    ADD R6,R6,#-1
  ...
  PS_EACH_SLOT_LOOP
    ADD R0,R4,#-16
    BRz PS_EACH_SLOT_DONE  ; if R0 == 16, break
  ...
  LD R7,PS_R7  ; recover R7
  RET

  PS_EMPTY_STR .STRINGZ ""
  PS_SEPARATOR_ASCII .FILL x7C  ; ascii '|'
  PS_NEWLINE_ASCII .FILL x0A
  PS_WEEKDAY_STR  ; each string use 4 memory locations (including NUL)
    .STRINGZ "Mon"
    .STRINGZ "Tue"
    .STRINGZ "Wed"
    .STRINGZ "Thu"
...
; PRINT_SLOT
;   This subroutine prints "XX:XX " to the screen, whose hour is the integer
...
PRINT_SLOT
  ...
  ADD R1,R1,#7  ; map R1 to 7:00 to 22:00
  AND R0,R0,#0  ; R0 = 0
...
; PRINT_CENTERED
;   This subroutine maps the string in R1 to 6 characters with possibly leading
...
PRINT_CENTERED
  ...
  ADD R2,R2,#-6
  BRnz PRINT_CENTERED_LE_SIX

  ; string length is greater than 6
  PRINT_CENTERED_G_SIX
    ...
    PRINT_CENTERED_ORIG
      ...
      ADD R1,R1,#1
      ADD R2,R2,#-1
      BRnzp PRINT_CENTERED_ORIG

  ; string length is less or equal to 6
  PRINT_CENTERED_LE_SIX
    ADD R2,R2,#6  ; recover R2
