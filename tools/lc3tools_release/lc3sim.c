 ...
 *
 * "Copyright (c) 2003-2021 by Steven S. Lumetta and LIU Tingkai."
 *
...
cmd_help (const char* args) 
{
    ...
    printf ("next                  -- execute next instruction (full "
    	    "subroutine/trap)\n");
    printf ("step [<count>]        -- execute one or <count> step(s) (into "
    	    "subroutine/trap)\n\n");
    ...
    printf ("rnext                 -- execute previous instruction (full "
    	    "subroutine/trap)\n");
    printf ("rstep [<count>]       -- execute one or <count> step(s) in reverse (into "
    	    "subroutine/trap)\n\n");
...
cmd_step (const char* args)
{
    ...
    if (*args == '\0') {
        ...
        char *endptr;
        count = strtoul(args, &endptr, 10);
        if (*endptr != '\0') {
            puts ("syntax: step [<count>]");
            return;
        }
    }

    flush_console_input ();
    for (; count > 0; count--) execute_instruction ();
    /* Dump memory and registers if necessary. */
    show_state_if_stop_visible ();
}
...
cmd_r_step (const char* args)
{
    ...
    if (*args == '\0') {
        ...
        count = strtoul(args, &endptr, 10);
        if (*endptr != '\0') {
            puts ("syntax: rstep [<count>]");
            return;
        }
    }
    flush_console_input ();
    for (; count > 0; count--) restore_step ();
	/* Dump memory and registers if necessary. */
	show_state_if_stop_visible ();
}
...
restore_step ()
{
    ...
    if (cur_step < 1 || !cached_step[idx].flags.valid ||
        cached_step[idx].flags.step != (int) cur_step) {
	if (gui_mode) {
